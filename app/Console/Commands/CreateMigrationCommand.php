...
use Illuminate\Support\Composer; // Required for MigrationCreator
use Illuminate\Support\Collection; // Added for easier collection handling
        ...
        foreach ($relationships as $relationName => $relationDef) {
            ...
            if (strtolower($type) === 'belongsto' && $relatedModel) {
                ...
                if (isset($relationDef['onDelete'])) {
                    ...
                    if ($action === 'cascade') $line .= "->cascadeOnDelete()";
                    elseif ($action === 'restrict') $line .= "->restrictOnDelete()";
                    elseif ($action === 'set null') $line .= "->nullOnDelete()"; // Column must be nullable
                    else $line .= "->onDelete('{$action}')"; // Raw action
                }
                if (isset($relationDef['onUpdate'])) {
                    $action = $relationDef['onUpdate'];
                    if ($action === 'cascade') $line .= "->cascadeOnUpdate()";
                    elseif ($action === 'restrict') $line .= "->restrictOnUpdate()";
                    else $line .= "->onUpdate('{$action}')";
                }
